...
int TCPProxy::makeClientSocket(const string &host, const uint &port) {
    ...
    dest.sin_family =
        PF_INET; /*storing the server info in sockaddr_in structure*/
...
#ifdef BIG_MSG
int TCPStub::sendBack(ProtoAddr addr, const void *sendbuf,
                      int sendcount) const {
    ...
    if (sentsize < sendcount) {
        ...
        // todo: bug prone
        /*	cerr << "TCPStub::sendBack():  error on
         BdSendToClient::bsend(...): "
         << strerror(errno) << endl;*/
    }
