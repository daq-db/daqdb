...
Blob::Blob(const string &blob) { assign(blob); }

Blob::Blob(const uint64_t &uuid, const size_t &seqNum, const size_t &total,
           const string &value)
    : _uuid(uuid), _seqNum(seqNum), _total(total), _value(value),
      _size(value.size()) {}

Blob::~Blob() {}

uint64_t Blob::uuid() const { return _uuid; }

void Blob::uuid(const uint64_t &uuid) { _uuid = uuid; }

size_t Blob::seqNum() const { return _seqNum; }

void Blob::seqNum(const size_t &seqNum) { _seqNum = seqNum; }

size_t Blob::total() const { return _total; }

void Blob::total(const size_t &total) { _total = total; }

string Blob::value() const { return _value; }

void Blob::value(const string &value) { _value = value; }

size_t Blob::size() const { return _size; }

void Blob::size(const size_t &size) { _size = size; }
...
uint Blob::getUuidLen() { return IdHelper::ID_LEN; }

uint Blob::getCountLen() { return 7; }

uint Blob::getSizeLen() { return 4; }
...
int BdSendBase::bsend(int sock) const { bsend(sock, NULL); }
...
int BdSendBase::bsend(int sock, void *senderAddr) const {
    ...
    for (VI it = _blobs.begin(); it != _blobs.end(); it++) {
        ...
        if (senderAddr == NULL)
            ...
            count +=
                sendto(sock, bstr.c_str(), bstr.size(), 0,
                       (struct sockaddr *)senderAddr, sizeof(struct sockaddr));

        /*remove the header, since the caller probably wants knowing
         * real-sent-size*/
...
BdSendToServer::BdSendToServer(const char *msg) : BdSendBase(msg) {}

BdSendToServer::BdSendToServer(const string &msg) : BdSendBase(msg) {}
...
BdSendToClient::BdSendToClient(const char *msg) : BdSendBase(msg) {}

BdSendToClient::BdSendToClient(const string &msg) : BdSendBase(msg) {}
...
template <class ID> AckQueue<ID>::AckQueue() {}

template <class ID> AckQueue<ID>::~AckQueue() {}

template <class ID> bool AckQueue<ID>::empty(const ID &id) const {
...
template <class ID> size_t AckQueue<ID>::size(const ID &id) const {
...
template <class ID> const ZPack &AckQueue<ID>::front(const ID &id) const {
...
template <class ID> const ZPack &AckQueue<ID>::back(const ID &id) const {
...
template <class ID> void AckQueue<ID>::push(const ID &id, ZPack ack) {
...
template <class ID> void AckQueue<ID>::pop(const ID &id) {
...
template <class ID> void AckQueue<ID>::rmQueue(const ID &id) {
