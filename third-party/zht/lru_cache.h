...
template <class T> struct Countfn {
...
template <class Key, class Data, class Sizefn = Countfn<Data>> class LRUCache {
  public:
    typedef std::list<std::pair<Key, Data>>
        List;                                  ///< Main cache storage typedef
  ...
  public:
     ...
     *  @note This function checks key existance and touches the key if it
     * already exists.
     */
    ...
    inline void insert(const Key &key, const Data &data, Data &removed_data) {
        ...
        while (_curr_size > _max_size) {
            // Remove the last element.
            //			cout<<"LRC_cache: hit the max_size "<<_max_size<<", now
            //current size = "<<_curr_size<<endl;
            liter = _list.end();
            --liter;
