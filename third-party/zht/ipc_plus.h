 ...
 *
 * This class is intended to be a base class. See the test program for a sample
 * derived class.
 */
...
class Msg {
  ...
  public:
     ...
     * \param key This is the "name" of the queue. By using the same value,
     * different processes can obtain access to the same message queue.
     * Obviously, some method must be deployed to both insure that this key is
     * unique in the system's shared queue space as well as known to those
     * processes that wish to share a queue. See ftok() for one possible method
     * to determine such a key value. \param createIfNotExist Should be pretty
     * evident.
     */
     ...
     * \param mType Optionally a priority or message type may be specified. This
     * <b>must</b> be a positive integer. \return False if this didn't work.
     */
    bool Snd(const void *data, const size_t size, const int mType = 1);
    /**
     * Simple send of a C-type string.
     *
     * Places a message in the queue with mSize==strlen(string) + 1 and
     * mdata==string. \param string The zero terminated string to send. \param
     * mType Optionally a priority or message type may be specified. This
     * <b>must</b> be a positive integer. \return False if this didn't work.
     */
     ...
     * Blocks until a message is available.
     * \param mType Optionally a priority or message type may be specified. This
     * <b>must</b> be a positive integer. \param mFlag See man msgrcv. \return
     * False if this didn't work.
     */
    bool RcvHigh() { return Rcv(-IPC_MIN_MSG_PRI); };
    /**
     ...
     * \return False if this didn't work.
     */
    bool RcvType(const int mType) { return Rcv(mType); };
    /**
 ...
 * use one queue to transmit "requests" and the other queue to receive
 * "replies".
 */
class TwoWayMsg {
  protected:
  ...
  public:
     ...
     *
     * \param key0 Will be the transmit queue and <b>key0 + 1</b> will be the
     * receive queue.
     */
     ...
     * <b>must</b> be a positive integer. \return False if this didn't work.
     */
    virtual bool xmitReq(const void *data, const size_t size,
                         const int mType = 1) {
     ...
     * <b>must</b> be a positive integer. \return False if this didn't work.
     */
    virtual bool xmitRep(const void *data, const size_t size,
                         const int mType = 1) {
...
/**
 * This class uses TwoWayMsg for the server side of a single server/multiple
 * client scenario.
 */
class MsgServer : public TwoWayMsg {
  ...
  public:
     ...
     * Constructor.
     */
    MsgServer(const unsigned int key0) : TwoWayMsg(key0), nextClient(0x100){};
    /**
     * Destructor.
     */
    virtual ~MsgServer(){
     ...
     * Receive a request.
     *
     * This methode automatically handles client registration in conjunction
     * with the MsgClient class below. Clients register with the server to
     * obtain a unique identification (address) which is handled transparantly
     * by this method. The server MUST call xmit() before calling recv() again
     * or the client will hang. \param mData Must be a pointer to a buffer of
     * the correct size. \param mSize Will hold the number of bytes received.
     * \return False if this didn't work.
     */
...
/**
 * This class uses TwoWayMsg for the client side of a single server/multiple
 * client scenario.
 */
class MsgClient : public TwoWayMsg {
  private:
     ...
     *
     * Initially set to zero and automatically assigned upon the first request
     * performed.
     */
    int myAddress;
  ...
  public:
     ...
     * Constructor.
     */
    MsgClient(const unsigned int key0) : TwoWayMsg(key0), myAddress(0){};
    /**
     * Destructor.
     */
    virtual ~MsgClient(){};
     ...
     * Transmit a request.
     *
     * The first time a client calls xmit, the registration with the server is
     * performed. \param data The bytes to send. \param size The number of bytes
     * to send. \return False if this didn't work.
     */
    virtual bool xmit(const void *data, const size_t size);
    /**
     * Receive a reply.
