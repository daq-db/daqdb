...
int ZHTClient::commonOp(const string &opcode, const string &key,
    ...
    if (opcode != Const::ZSC_OPC_LOOKUP && opcode != Const::ZSC_OPC_REMOVE &&
        opcode != Const::ZSC_OPC_INSERT && opcode != Const::ZSC_OPC_APPEND &&
        opcode != Const::ZSC_OPC_CMPSWP && opcode != Const::ZSC_OPC_STCHGCB &&
        opcode != Const::ZSC_OPC_PING)
...
string ZHTClient::commonOpInternal(const string &opcode, const string &key,
    ...
    ZPack zpack;
    zpack.set_opcode(opcode); //"001": lookup, "002": remove, "003": insert,
                              //"004": append, "005", compare_swap
    ...
    if (val.empty()) {
        ...
        zpack.set_val(
            "^"); // coup, to fix ridiculous bug of protobuf! //to debug
    ...
    if (srecv.empty()) {
        ...
        result = srecv.substr(
            3); // the left, if any, is lookup result or second-try zpack
        sstatus = srecv.substr(
            0, 3); // status returned, the first three chars, like 001, -98...
    }
