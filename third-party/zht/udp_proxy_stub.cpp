...
UDPProxy::UDPProxy() : SOCK_CACHE(), ADDR_CACHE() { init_AC_MUTEX(); }
...
bool UDPProxy::sendrecv(const void *sendbuf, const size_t sendcount,
    ...
    /*receive response from server over client sock fd*/ // todo: loopedReceive
                                                         // for zht_lookup
...
int UDPProxy::recvFrom(int sock, void *recvbuf) {
    ...
    int recvcount = ::recvfrom(sock, buf, sizeof(buf), 0,
...
#ifdef BIG_MSG
int UDPStub::sendBack(ProtoAddr addr, const void *sendbuf,
                      int sendcount) const {
...
int UDPStub::sendBack(ProtoAddr addr, const void *sendbuf,
    ...
    // send response to client over server sock fd
    int sentsize =
        ::sendto(addr.fd, sendbuf, sendcount, 0, (struct sockaddr *)addr.sender,
                 sizeof(struct sockaddr));
