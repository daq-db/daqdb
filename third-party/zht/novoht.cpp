...
NoVoHT::NoVoHT(const string &f) { initialize(f, 1000, -1, 0); }
...
int NoVoHT::remove(string k) {
    ...
    if (k.compare(cur->key) == 0) {
        ...
        nRem++;
        if (nRem == magicNumber)
            ret += writeFile(); // write and save write success
        sem_post(&map_lock);
        return ret;
    }
    while (cur != NULL) {
        if (cur->next == NULL) {
            ...
            nRem++;
            if (nRem == magicNumber)
                ret += writeFile(); // mark and sace status code
            sem_post(&map_lock);
            return ret;
        }
        cur = cur->next;
    ...
    for (int i = 0; i < size; i++) {
        ...
        while (cur != NULL) {
            if (!cur->key.empty() && !cur->val.empty() && !cur->diff) {
                ...
                fprintf(swapFile, "%s\t%s\t", cur->key.c_str(),
                        cur->val.c_str());
            }
            cur = cur->next;
        }
...
bool NoVoHT::isRewriting() const { return rewriting; }

int NoVoHT::flushDbfile() { return fflush(dbfile); }

key_iterator NoVoHT::keyIterator() { return key_iterator(kvpairs, size, this); }
val_iterator NoVoHT::valIterator() { return val_iterator(kvpairs, size, this); }
