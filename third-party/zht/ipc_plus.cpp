...
bool MsgClient::xmit(const void *data, const size_t size) {
    if (!myAddress) {
        ...
        size_t dummy;
        // Note that this may be the reply to the registration request of
        // another process. This does not matter as in that case the other
        // process will obtain the reply to our registration request.
