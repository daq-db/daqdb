...
Daqdb supports offloading of data to SSD NVMe storage. For the performance reasons it's recommended to use NVMe Optane drives since those support very high IO and GB/s rates for types of loads that are generated by daqdb.

Offload is enabled via config.spdk file, which must be copied to the directory below ${daqdb_path}

Current release supports two types of drive congurations:
- Single drive
- JBOD 

An exampe Single drive config.spdk is shown below:
```
offload_unit_alloc_size = 1024
offload_dev_name = "example_offload"
offload_dev_type = "bdev"
offload_nvme_addr = "0000:89:00.0"
offload_nvme_name = "Nvme1"
```

A JBOD config example is provided below:
```
offload_unit_alloc_size = 1024
offload_dev_name = "example_offload_jbod"
offload_dev_type = "jbod"
devices = {
...
```
config.spdk examples can be found in ${daqdb_path}/lib/offload folder.
